module leviathan::card_poker_game {
    ...
    public entry fun start_game(
        ...
        inst.deck = new_deck(template.num_suits, template.ranks_per_suit);
        shuffle(&mut inst.deck, r, ctx); 
        inst.hands = deal_hands(&mut inst.deck, vector::length(&inst.players), template.cards_per_hand);
        inst.started = true;
    }
    ...
    public entry fun finalize(
        ...
        inst.winners = evaluate_all(&inst.players, &inst.hands, template.combination_size, template.victory_mode, template.ranks_per_suit);
        inst.ended = true;
