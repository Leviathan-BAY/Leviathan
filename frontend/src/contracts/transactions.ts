...
export function createFullGameTemplate(
  name: string,
  description: string,
  diceMin: number,
  diceMax: number,
  ...
  tx.moveCall({
    target: `${PACKAGE_ID}::board_game_maker::create_full_game_template`,
    arguments: [
      tx.pure.string(name),
      ...
      tx.pure.vector('u8', board),
      tx.pure.vector('u8', startPositions),
      tx.pure.vector('u8', finishPositions),
    ],
  });
